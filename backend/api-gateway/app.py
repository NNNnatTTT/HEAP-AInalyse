...
MATCH_HISTORY_SERVICE = "http://match-history-service:5000"
AUTH_SERVICE = "http://auth-service:5000"

@app.get("/match-history/")
async def proxy_match_history():
    async with httpx.AsyncClient() as client:
        response = await client.get(f"{MATCH_HISTORY_SERVICE}/matches")
...
async def proxy_auth_login(request: Request):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(f"{AUTH_SERVICE}/login", json=json_data)
    return response.json()
